...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; color: #666; font-size: 14px;">
    ...
    console.error(`❌ Failed to send approval email to ${userEmail}:`, error.message);
    return false;
  }
}
 ...
 * @returns {Promise<boolean>} Success status
 */
export async function sendRejectionEmail(userEmail, userName, bookingDetails, rejectionReason) {
  try {
    const { roomName, date, startTime, endTime, duration, purpose } = bookingDetails;
    const formattedDate = formatDate(date);
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; color: #666; font-size: 14px;">
    ...
    console.error(`❌ Failed to send rejection email to ${userEmail}:`, error.message);
    return false;
  }
}
